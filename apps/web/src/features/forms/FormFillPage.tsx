...
export default function FormFillPage() {
  const { id = "" } = useParams();
  return apiConfigured ? <LiveFormFill id={id} /> : <MockFormFill id={id} />;
}
...
function Sealed({ name }: { name: string }) {
  return (
    <div className="mx-auto max-w-xl py-12 text-center">
      ...
      <Link to="/forms" className="mt-6 inline-flex rounded-xl border border-edge bg-white px-5 py-2.5 text-sm font-semibold text-olive-deep hover:bg-bone-light">
        Back to forms
      </Link>
    </div>
  );
...
function LiveFormFill({ id }: { id: string }) {
  ...
  if (q.error || !q.data) return <Navigate to="/forms" replace />;
  ...
  return (
    <div className="mx-auto max-w-5xl">
      <Link to="/forms" className="mb-3 inline-flex items-center gap-1.5 text-sm text-muted hover:text-olive-deep">
...
function MockFormFill({ id }: { id: string }) {
  ...
  if (!form) return <Navigate to="/forms" replace />;
