...
// transcribeField uses a module-level `prisma` import (unlike listByStage/
// getById/create, which take prisma as a parameter), so it needs its own
// vi.mock('@bin-tracker/db') file rather than living in form.service.test.ts.
...
describe('formService.transcribeField metering', () => {
    it('rejects with TOO_MANY_REQUESTS once the org is at its plan monthlyTranscribe limit', async () => {
        ...
        await expect(
            formService.transcribeField({ audioBase64: 'abc', fieldLabel: 'Name' } as never, 'org-a'),
